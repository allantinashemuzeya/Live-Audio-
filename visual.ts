...
export class GdmLiveAudioVisuals extends LitElement {
  ...
  private visualize() {
    if (this.canvas && this.inputAnalyser && this.outputAnalyser) {
      ...
      // Draw output visualization first as a base
      ...
      for (let i = 0; i < this.outputAnalyser.data.length; i++) {
        const barHeight = (this.outputAnalyser.data[i] / 255) * HEIGHT;
        canvasCtx.fillRect(x, HEIGHT - barHeight, outputBarWidth, barHeight);
        x += outputBarWidth;
      }

      // Use a different composite operation to blend the input visualization
      canvasCtx.globalCompositeOperation = 'screen';

      // Draw input visualization on top, with a different style
      ...
      for (let i = 0; i < this.inputAnalyser.data.length; i++) {
        const barHeight = (this.inputAnalyser.data[i] / 255) * HEIGHT;
        // Make input bars thinner to distinguish them
        const thinBarWidth = inputBarWidth / 2;
        canvasCtx.fillRect(
          x + thinBarWidth / 2,
          HEIGHT - barHeight,
          thinBarWidth,
      ...
      // Reset composite operation to default for the next draw cycle
